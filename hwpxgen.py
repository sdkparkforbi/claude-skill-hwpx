...
# ── 셀 여백 기본값(HWPUNIT) — 기존 340/340/100/100은 너무 답답해 넉넉하게 상향 ──
#    left, right, top, bottom (1mm ≈ 283 HWPUNIT). 셀 안 텍스트가 테두리에 붙는 문제 해소.
CELL_MARGIN = (510, 510, 312, 312)   # ≈ 좌우 1.8mm, 상하 1.1mm

def _norm_margin(m):
    """여백 인자 정규화 → (left,right,top,bottom). int=네 변 동일, (lr,tb)=2튜플, (l,r,t,b)=4튜플, mm은 mm()로 변환해 전달."""
    if m is None: return CELL_MARGIN
    if isinstance(m, (int, float)): v = int(round(m)); return (v, v, v, v)
    m = list(m)
    if len(m) == 2: lr, tb = int(round(m[0])), int(round(m[1])); return (lr, lr, tb, tb)
    if len(m) == 4: return tuple(int(round(x)) for x in m)
    raise ValueError('margin은 int 또는 (lr,tb) 또는 (l,r,t,b)')

def _set_cell_height(cell_xml, h):
    """셀 XML의 cellSz height를 h로 교체(표 늘이기/stretch용). 행복제 없이 안전."""
    return re.sub(r'(<hp:cellSz width="\d+" height=")\d+(")', r'\g<1>%d\g<2>' % int(h), cell_xml, count=1)
...
class HwpxDoc:
    ...
    def cell(self, lines, col, row, width, height=900, bf=BF_DATA, paraPr=PA_CENTER, colspan=1, rowspan=1, margin=None, vertAlign='CENTER'):
        # lines: str 또는 (text,charPr) 리스트; 여러 줄이면 hp:p 여러 개
        # margin: 셀 안쪽 여백. None=기본(CELL_MARGIN), int=네 변, (lr,tb), (l,r,t,b). mm은 mm()로 변환.
        # vertAlign: 셀 세로 정렬 TOP/CENTER/BOTTOM.
        ...
        ml,mr,mt,mb=_norm_margin(margin)
        return (f'<hp:tc name="" header="0" hasMargin="1" protect="0" editable="0" dirty="0" borderFillIDRef="{bf}">'
                f'<hp:subList id="" textDirection="HORIZONTAL" lineWrap="BREAK" vertAlign="{vertAlign}" '
    ...
    def table(self, rows, col_widths, treat_as_char=True, cell_margin=None,
              row_height=None, total_height_mm=None, pageBreak='CELL'):
        """표 생성.
        treat_as_char: True=글자처럼 인라인(기본), False=글자객체로 인식 안 함(쪽 경계에서 셀 단위로 나뉘어 잘 안 깨짐).
        cell_margin: 표 기본 셀 여백(inMargin). None/int/(lr,tb)/(l,r,t,b).
        row_height / total_height_mm: 표를 세로로 늘임(끌어당기기). row_height=각 행 높이(HWPUNIT),
                                      total_height_mm=표 전체 높이(mm)를 행수로 등분. baking이 줄어들지 않게 행높이를 고정.
        pageBreak: 'CELL'(쪽 경계에서 셀 단위 나눔=권장), 'TABLE'(통째 이동), 'NONE'.
        """
        rc=len(rows); cc=len(col_widths); tw=sum(col_widths)
        # ── 표 늘이기(stretch): 각 셀 높이 고정 ──
        if total_height_mm is not None and rc:
            row_height = int(round(mm(total_height_mm)/rc))
        tbl_h = 1282
        if row_height is not None:
            rows = [[_set_cell_height(c, row_height) for c in r] for r in rows]
            tbl_h = row_height*rc
        tid=self._tbl_id; self._tbl_id+=1
        z=self._z; self._z+=1
        tac = '1' if treat_as_char else '0'
        il,ir,it,ib=_norm_margin(cell_margin)
        trs=''.join('<hp:tr>'+''.join(r)+'</hp:tr>' for r in rows)
        tbl=(f'<hp:tbl id="{tid}" zOrder="{z}" numberingType="TABLE" textWrap="TOP_AND_BOTTOM" '
             f'textFlow="BOTH_SIDES" lock="0" dropcapstyle="None" pageBreak="{pageBreak}" repeatHeader="1" '
             f'rowCnt="{rc}" colCnt="{cc}" cellSpacing="0" borderFillIDRef="{BF_DATA}" noAdjust="0">'
             f'<hp:sz width="{tw}" widthRelTo="ABSOLUTE" height="{tbl_h}" heightRelTo="ABSOLUTE" protect="0"/>'
             f'<hp:pos treatAsChar="{tac}" affectLSpacing="0" flowWithText="1" allowOverlap="0" holdAnchorAndSO="0" '
